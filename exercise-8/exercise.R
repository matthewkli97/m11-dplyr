...
pulitzer <- mutate(pulitzer, Pulitzer.Prize.Change = Pulitzer.Prize.Winners.and.Finalists..2004.2014 - 
...
most.2014 <- pulitzer %>% filter(max(Pulitzer.Prize.Winners.and.Finalists..2004.2014) == 
...
publication <- pulitzer %>% filter(Pulitzer.Prize.Winners.and.Finalists..2004.2014 >= 5) %>% filter(min(Change.in.Daily.Circulation..2004.2013) ==
...
# Which publications has the biggest increase in pulitzer prizes from 1990.2014

biggest.increase <- pulitzer %>% filter(max(Pulitzer.Prize.Change) == Pulitzer.Prize.Change) %>% select(Newspaper)
